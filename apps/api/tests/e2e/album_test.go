...
// uniquePhotoSuffix keeps two photos uploaded in the same run from sharing a
// digest. The tests here run in parallel, so it has to be atomic.
var uniquePhotoSuffix atomic.Int64

// photoRunID does the same across runs, which the counter alone cannot: it
// restarts at one in every process, while the database remembers the digest an
// earlier run registered and hands the photo back the key that run stored it
// under -- in a bucket built for that run and gone by this one.
...
//
// Every photo gets bytes of its own. Identical bytes are deduplicated onto one
// storage_objects row whose key belongs to whichever photo registered the
// digest first -- and since the test database outlives a run while the bucket
// does not, that key points into a bucket that no longer exists the next time
// round. Sharing is a property worth testing, but only where a test asks for
// it: see album_storage_object_test.go, which supplies identical bytes on
// purpose.
func uploadOnePhoto(t *testing.T, tt *helpers.TestTenant, body map[string]any) albumPresignResp {
	t.Helper()
	png := append(helpers.TinyPNG(), fmt.Appendf(nil, "%d-%d", photoRunID, uniquePhotoSuffix.Add(1))...)
...
func TestAlbumDownloadURL(t *testing.T) {
	...
	assert.Equal(t, pres.Bytes, body)
