...
function SharedCalendarView() {
  ...
  if (error) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--color-surface-secondary)]">
        <div className="rounded-2xl bg-[var(--color-surface)] px-8 py-12 text-center shadow-sm">
          <svg
  ...
  if (!calendar) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--color-surface-secondary)]">
        <div className="text-[14px] text-[var(--color-text-secondary)]">{t('common.loading')}</div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen flex-col bg-[var(--color-surface-secondary)]">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[16px] font-bold sm:text-[18px]">{calendar.name}</h1>
        </div>
              ...
              return (
                ...
                >
                  <div className="mb-1 flex items-center justify-center">
                    <span
                      className="flex h-6 w-6 items-center justify-center rounded-full text-[12px] sm:text-[13px]"
                      style={{
                  ...
                  {dayEvents.slice(0, 3).map((evt) => (
                    <div
                      key={evt.id}
                      className="mb-0.5 truncate rounded px-1 py-0.5 text-[10px] leading-tight text-white sm:text-[11px]"
      ...
      <div className="bg-[var(--color-surface)] px-4 py-5 shadow-inner sm:px-6">
        {joinError && (
          <div className="mb-3 rounded-lg bg-red-50 px-4 py-3 text-center text-[13px] text-red-600">
            {joinError}
          </div>
        )}
        <button
          ...
          onClick={handleJoin}
          disabled={joining}
          className="h-11 w-full rounded-lg bg-[var(--color-accent)] text-[15px] font-bold text-white transition-opacity hover:opacity-90 disabled:opacity-50"
        >
      ...
      {/* Footer */}
      <footer className="bg-[var(--color-surface)] py-4 text-center text-[12px] text-[var(--color-text-tertiary)]">
        {t('share.poweredBy')}
      </footer>
    </div>
  );
